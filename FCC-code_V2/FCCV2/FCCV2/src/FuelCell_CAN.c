...
void can_out_callback_channel0(U8 handle, U8 event)
{
	...
	if (can_mob_get_status(0,handle) == CAN_STATUS_COMPLETED)
	{
...
U8 can_send_message(CanMessage * p_message)
{
	...
	fc_tx_mob.handle = can_mob_alloc(0); //get handle of a free mob
	...
	can_tx(0, fc_tx_mob.handle, fc_tx_mob.dlc, fc_tx_mob.req_type,fc_tx_mob.can_msg);
...
{
	U8 status = can_mob_get_status(0,handle);
	if (status == CAN_STATUS_COMPLETED) //message has sent
	{
		//clear mob status
		can_clear_status(0,handle);
		//free mob
		can_mob_free(0,handle);
		return(0);
	}
