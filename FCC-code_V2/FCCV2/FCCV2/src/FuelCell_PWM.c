 ...
 * FuelCell_PWM.c
 *
 * Created: 2015-11-10 10:16:54 AM
 *  Author: Reegan
 */ 
...
static void local_start_highfreq_clock(void)
{
	const scif_pll_opt_t opt = {
		...
		//these values are probably wrong (I changed clock but didn't change them)
...
static void pwm_start_gc(void)
{
	...
	scif_gc_setup(AVR32_SCIF_GCLK_PWM,SCIF_GCCTRL_PLL0,AVR32_SCIF_GC_DIV_CLOCK,249);
...
vavr32_pwm_channel_t fan_pwm_channel = {
	{0}, // cmr
	{0}, // cdty
	{0}, // cdtyupd
	{0}, // cprd
...
void PWMInit(void)
{
	...
	//Servo Channel configuration these have not been adjusted yet
	...
	pwm_channel.CMR.cpre  = 10;           // Channel prescaler divide by 1024 = 97.65Hz should work for servo see page 1036 of datasheet.
	...
	pwm_channel_init(SERVO_PWM_CHANNEL_ID, &servo_pwm_channel); // Set channel configuration to channel 3
	pwm_channel_init(FAN_PWM_CHANNEL_ID, &fan_pwm_channel); // Set channel configuration to channel 2
	pwm_start_channels((1 << SERVO_PWM_CHANNEL_ID)|(1 << FAN_PWM_CHANNEL_ID));  // Start channel 2 & 3.
}
