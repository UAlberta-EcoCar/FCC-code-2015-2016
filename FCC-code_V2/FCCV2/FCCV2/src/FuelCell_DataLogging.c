...
void usart_can_bridge(unsigned int fc_state, unsigned int error_msg,unsigned int past_fc_state)
{
	if(millis()-usart_bridge_timer > USART_BRIDGE_INTERVAL)
	{
		...
		sprintf(str,"%llu,%llu,%llu,%d,%d,%d,",get_total_E(),get_coulumbs_since_last_purge(),get_total_charge_extracted(),get_FCVOLT()/1000,get_FCCURR()/1000, get_CAPCURR()/1000);
		usart_write_line(OPEN_LOG_USART,str);
		...
		sprintf(str,"%d,%d,%d,%d,%d,%d\n\r",gpio_get_gpio_pin_output_value(RES_RELAY),gpio_get_gpio_pin_output_value(CAP_RELAY),gpio_get_gpio_pin_output_value(MOTOR_RELAY),gpio_get_gpio_pin_output_value(PURGE_VALVE),gpio_get_gpio_pin_output_value(H2_VALVE),past_fc_state);
		usart_write_line(OPEN_LOG_USART,str);
